...
router.get("/get/:username", (req, res) => {
  ...
  console.log(recipesList, userList);
...
router.post("/post/:username", (req, res) => {
  ...
  res
    .status(200)
    .send(`Recipe successfully posted to ${req.params.username}'s database`);
});

router.get("/delete/:username/:id", (req, res) => {
  console.log(
    `Deleting recipe of user: ${req.params.username} with id: ${req.params.id}`
  );

  recipesList[userList.indexOf(req.params.username)] = recipesList[
    userList.indexOf(req.params.username)
  ].filter((recipe) => recipe.id != req.params.id);
