myApp.controller('RecipeCreationController', ["$scope", "$state", "$http", "RecipesService",
    ...
    function ($scope, $state, $http, recipesService) {
        ...
        cantReachServerAlert = function(){
            alert("Could not reach the server, you will not be able to create a new recipe at this time");
        ...
        $scope.reloadItemsList = function () {
            ...
            $http({
                ...
                items = response.data;
                $scope.ahData = "";
                bindFormsToAutocompletionList(items);
        ...
        function autocomplete(inp, arr) {
            ...
            inp.addEventListener("input", function (e) {
                ...
                for (i = 0; i < arr.length; i++) {
                    ...
                    if (arr[i]._name.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
                        /*create a DIV element for each matching element:*/
                        b = document.createElement("DIV");
                        /*make the matching letters bold:*/
                        b.innerHTML = "<strong>" + arr[i]._name.substr(0, val.length) + "</strong>";
                        b.innerHTML += arr[i]._name.substr(val.length);
                        /*insert a input field that will hold the current array item's value:*/
                        b.innerHTML += "<input type='hidden' value='" + arr[i]._name + "'>";
        ...
        function addItem(list,form){
            ...
            if(itemIndex != -1){
                ...
                list.push({
                    ...
                    count: 1,
                    id: items[items.findIndex(item => item.name == document.getElementById(form).value)].id
                });
            }} catch(err){
                console.log(err);
